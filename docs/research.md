...
## 待实拍清单（Phase 6 前）
- ~~xept /projects 内页~~：登录墙无法通过（2026-09-10 尝试 cookie 注入与内嵌浏览器登录均失败），已有登录/注册页 `[实拍]` 与样式提取足够定设计语言，不再阻塞
- W&B 单个 run 页面布局（日志+参数+指标三栏比例）—— `[先验]` 兜底：按 MLOps 通用三栏范式
- Connected Papers 画布交互细节（节点 hover 卡、边高亮）—— `[先验]` 兜底：力导向画布通用交互

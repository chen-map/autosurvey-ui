...
| 日期 | 变更 | 影响面 | 提出人 |
|---|---|---|---|
| 2026-09-24 | **W5 部署完成（结构打通，正文为存量示范文——如实标注）**。存量 run_workflow5.py 是无 LLM 的确定性 LaTeX 装配器（1030 行）：消费 W3 大纲/矩阵 + W4 工作记忆 + 结构化论文 → IEEE 综述（main.tex + 9 sections + tables/figures + references.bib 65 条/41 被引 + 自审报告）。接入方案：w5/stage_workspace.py 装配工作台（本项目布局 → 存量期望布局 + PAPER_INDEX.csv 生成）+ w5/phase_defs 两 Phase（装配/成文）。**前置补齐**：W4 新增 P2B 阶段（build_working_memory + generate_answer_claims per RQ——W5 输入契约，漏跑导致 payloads KeyError）。**如实标注的局限**：摘要/叙事为存量硬编码示范文（记忆安全域），正文的 LLM 化写作（paper-write SKILL 驱动）为下一步；section 文件名为存量硬命名。**全流水线 W1→W5 五条工作流全部真实跑通** | backend/w5/stage_workspace.py、w5/phase_defs.py（新）、backend/w4/phase_defs.py（P2B）、runner/后端/前端扩 w5 | 用户 || 日期 | 变更 | 影响面 | 提出人 |
|---|---|---|---|
| 2026-09-24 | **W4 部署完成并真实跑通**（用户：开工W4，以文档+源代码为主）。存量 W4 三阶段映射为执行器三 Phase（per-RQ 展开）：P1 证据抽取（extract_rq_evidence，LLM 逐篇读结构化卡）→ P2 答案综合（synthesize_rq_answer，RQ 类型判定 + 维度组织 + 跨论文共识/分歧/演化）→ P3 claim 四维核查（证据可追溯/详实度/引用完整性/一致性）+ WORKING_MEMORY_INDEX。**LLM 注入新机制**：W4 脚本以别名 module_from_spec 加载 kg_common（硬编码 skills/auto_survey_skills/... 路径），llm_wrap 补丁不可达 → ① 硬编码路径部署 kg_common shim（转发 W2 真身 + AS_LLM_* 环境变量覆盖）；② runner run_step 支持 step["env"] 注入；③ w4/phase_defs 经 llm_wrap.load_llm_config 解析 DB 配置写入 step env。实跑（100 篇新语料，4 Macro）：110+ 次 LLM 调用，rq_1..rq_4 工作记忆齐备——RQ1 五维度/4 共识/0.72 置信度，claim 核查 12 条（8 pass/4 needs_revision/0 reject，pass_rate 0.667，usability needs_revision——锚定机制如实标记待修订项）。前端流水线页新增 W4 标签 | backend/w4/phase_defs.py（新）、runner.py（step env）、skills/.../kg_common.py shim（存量库）、PipelinePage、api.ts | 用户 || 日期 | 变更 | 影响面 | 提出人 |
|---|---|---|---|
...
| 日期 | 变更 | 影响面 | 提出人 |
|---|---|---|---|
| 2026-09-23 | **纪要方向获授权改动（备份先行）**：① 新增通用备份工具 kg_backup.py + References 回填 backfill_references.py（strict 节标题 + 分离字母 "R EFERENCES" 变体 + paper_text 尾切三策略，幂等只填空壳，自动备份）——实质书目 31→36/100；诊断确认本批语料（关键词筛选的随机子集）内部互引≈0，引用边不出的主因是语料性质而非代码。② 新增 kg_merge_execute.py AutoMerge 执行（保守模式：仅精确同名，dry-run 预览 → 备份 → 同步改 JSON+DB，别名入 node_aliases，边重接+平行边去重）——实测合并 2 组（Cifar-10←CIFAR10、F1 score←F1-score），JSON 885→883/DB 1042→1040，前端 /kg 实时生效。词元相似候选 6 对留人工/LLM 复核（Hits@1↔Hits@k 等假阳性印证"向量+LLM 规则"必要性）。③ 其余方向（章节定位提取/Pre-score 精细化/图数据库/Skill 体系）维持登记待裁决 | backend/w2/{kg_backup,backfill_references,kg_merge_execute}.py（新）、本项目 paper_cards 与 KG 数据（备份在 _backup/） | 用户 |
| 2026-09-23 | **登记 W2/W3 优化方向会议纪要**（用户裁决：只登记；轻微改动可尝试；改文件类型/存储格式暂不动）。纪要归档 references/（五项 KG 优化：Pre-score 精细化/章节定位提取/Related Work 引用解析/图数据库调研/AutoMerge；两项架构：三层分析架构、Problem-Solution 双域 Skill 体系）。backend-todo 新增 §9 逐项现状对照表。**附带诊断**：Related Work 短板拿到硬证据——100 卡仅 31 张有 references 字段且全部 ≤50 字符（解析层空壳），citation_index 无米下锅。**轻量试点（用户允许范围内）**：新增 w2/kg_merge_suggest.py（纯只读），真实 KG 产出合并候选报告——精确同名 2 对（Cifar-10↔CIFAR10、F1 score↔F1-score）+ 词元相似 6 对；假阳性（Hits@1↔Hits@k）恰好印证纪要"向量+LLM 规则判定"必要性 | docs/references/纪要归档、backend-todo §9、backend/w2/kg_merge_suggest.py（新，只读） | 用户 |

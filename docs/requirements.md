...
| B2-2 | 主题输入 | 大输入框 + 帮助文案（"输入完整的研究主题或选题关键词，获得更好的生成效果"式） | S | 用户提出 | ✅ |
...
| 编号 | 功能点 | 交互描述 | 工作量 | 来源 | 裁决 |
|---|---|---|---|---|---|
| B13 | 研究方向画像（静态版） | 领域标签库管理（与向导共享自定义标签）+ 研究目标表单；创建向导自动预选本次组合，双向共享 localStorage | S | 用户裁决(F17) | ✅ |
| B14 | AI 方向精炼（mock 版） | 方向页内：模糊 prompt → 结合领域标签精炼为专业研究方向（阐述/研究问题/Gap）+ 推荐论文 3 篇，可一键采纳为研究目标；真实实现（后端 LLM 代理 + Semantic Scholar 检索）见 backend-todo.md §3 | M | 用户裁决(F18 升级进本版) | ✅ |

    ...
    macros: [
      { id: 'RQ1', text: '哪些 GNN 可解释性方法已被提出，其原理如何分类？', subs: [
        { id: 'rq1-1', text: '掩码/扰动类方法的原理演进与代表工作', score: 0.92, level: 'strong', paperCount: 12,
          ...
          motivation: 'Gap 分析（W3-P1）显示现有综述普遍把掩码/扰动/梯度三类方法混在"归因"名下，缺少按解释单元粒度（边/节点/子图）的谱系梳理；本 RQ 负责该章节的方法学骨架。',
          definition: '覆盖以边/节点/子图掩码与扰动为核心机制的方法（GNNExplainer、PGExplainer、SubgraphX 等）；不含纯梯度归因（归 rq1-2）与代理模型方法。',
        ...
        { id: 'rq1-2', text: '梯度归因方法在图结构上的适配方式', score: 0.88, level: 'strong', paperCount: 9,
          rqType: 'descriptive', chapter: '§3 掩码与扰动方法',
          motivation: '梯度法在消息传递结构上的适配（如 GNNGradExplainer 的邻域聚合改写）常被忽略；引文图上梯度方法仍有竞争力但综述覆盖弱，属 coverage gap。',
          definition: '覆盖基于梯度显著性/归因的方法及其对消息传递结构的改造；不含训练中干预类方法。',
      ...
      { id: 'RQ2', text: '各类方法在基准数据集上的表现如何横向比较？', subs: [
        { id: 'rq2-1', text: '合成基准（BA-Shapes 等）上的定量对比', score: 0.86, level: 'strong', paperCount: 7,
          ...
          motivation: '各方法论文自报指标的实验口径不一；跨论文统一定量对比是本综述的差异化贡献点之一，也是跨论文一致性核查的重点区。',
          definition: '仅纳入在同一合成基准（BA-Shapes、BlogCatalog 等）+ 相同数据划分下报告 Fidelity+/Sparsity 的结果；不含作者自定义数据集。',
        ...
        { id: 'rq2-2', text: '真实引文图上的跨数据集泛化', score: 0.81, level: 'weak', paperCount: 5,
          rqType: 'comparative', chapter: '§4 基准评估',
          motivation: '合成基准上的结论能否迁移到真实引文图决定实践价值；现有证据集中在合成侧，真实图证据不足是 weak 评分的直接原因。',
          definition: '纳入在 Cora/Citeseer/PubMed 等真实引文图上的对比结果；跨数据集结论要求 ≥2 个真实图方计入。',
      ...
      { id: 'RQ3', text: '现有方法面临哪些限制与开放问题？', subs: [
        { id: 'rq3-1', text: '解释稳定性与对抗鲁棒性', score: 0.9, level: 'strong', paperCount: 6,
          ...
          motivation: '解释的稳定性（扰动不变性）评估普遍缺失；对抗视角近两年快速升温，现有综述极少覆盖，属新兴 gap。',
          definition: '覆盖解释稳定性度量与对抗攻击下解释的鲁棒性；不含 GNN 模型本身的鲁棒性研究。',
        ...
        { id: 'rq3-2', text: '异构图上的可解释性', score: 0.47, level: 'blocked', paperCount: 2,
          ...
          motivation: '应用侧（引文图/知识图谱）多为异构图，但方法侧几乎全部为同构图设计；候选 gap，证据是否充分待 P4 修订循环裁决。',
          definition: '覆盖异质图（HIN/知识图谱）上的解释方法；当前 KG 证据仅 2 篇，低于每 RQ ≥5 篇阈值，故 blocked。',
      ...
      { id: 'RQ4', text: '评估指标体系如何构成，存在哪些缺陷？', subs: [
        { id: 'rq4-1', text: 'Fidelity/Sparsity 指标族的适用边界', score: 0.87, level: 'strong', paperCount: 5,
          ...
          definition: '覆盖 Fidelity+/−、Sparsity、Contrastivity、Stability 及其变体的定义与适用条件；不含人类评估指标。',

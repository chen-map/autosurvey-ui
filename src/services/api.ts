...
// ---- pipeline ----
export async function startRun(projectId: string, workflow: 'w1' | 'w2' | 'w3' | 'w4' = 'w1'): Promise<void> {
  if (!USE_MOCK) {
...
const WORKFLOW_META: Record<'w1' | 'w2' | 'w3' | 'w4', { id: string; name: string }> = {
...
export async function getPipeline(projectId: string, workflow: 'w1' | 'w2' | 'w3' | 'w4' = 'w1'): Promise<PipelineRun> {
  if (USE_MOCK) {
    await delay();

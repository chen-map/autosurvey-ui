...
export function KnowledgeBasePage() {
  const { items, collections, remove, moveTo, addCollection } = useLibrary();
      ...
      {items.length === 0 ? (
        <Card className="mt-6 flex flex-col items-center gap-3 px-8 py-16 text-center">
          ...
          <p className="max-w-md text-[13px] leading-5 text-t3">
            在任意项目的「语料库」中点击论文行内的收藏按钮，即可保存到这里并分类管理。
          </p>
          ...
          {shown.map((i) => (
            <Card key={i.key} className="flex items-center gap-4 p-4 transition-shadow hover:shadow-s2">
              ...
              <div className="min-w-0 flex-1">
                <div className="truncate text-[14px] font-medium text-t1">{i.title}</div>
                <div className="mt-0.5 text-[12px] text-t3">
                  {i.authors} · {i.venue} {i.year} · 被引 {i.citations} · 收藏于 {i.savedAt}
                </div>
              </div>
              <select
                value={i.collection}
